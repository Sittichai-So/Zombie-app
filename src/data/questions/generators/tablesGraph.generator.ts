...
import { TablesGraphQuestion } from '../tablesGraph';
...
// Easy Questions: อ่านตาราง/กราฟพื้นฐาน
const generateEasyQuestions = (): TablesGraphQuestion[] => {
  const questions: TablesGraphQuestion[] = [];
...
ใครได้คะแนนสูงสุด?`,
      table: [
        ['ชื่อ', 'สมชาย', 'สมหญิง', 'วิชัย', 'มานี', 'ปิติ'],
        ['คะแนน', scores[0].toString(), scores[1].toString(), scores[2].toString(), scores[3].toString(), scores[4].toString()]
      ],
      chartType: 'table',
...
const generateMediumQuestions = (): TablesGraphQuestion[] => {
  const questions: TablesGraphQuestion[] = [];
...
ค่าใช้จ่ายหมวดอาหารคิดเป็นกี่เปอร์เซ็นต์ของทั้งหมด?`,
      table: [
        ['หมวดหมู่', 'อาหาร', 'เสื้อผ้า', 'ที่อยู่อาศัย', 'การเดินทาง', 'อื่นๆ'],
        ['จำนวนเงิน', expenses[0].toString(), expenses[1].toString(), expenses[2].toString(), expenses[3].toString(), expenses[4].toString()]
      ],
      chartType: 'table',
      options: [
        `${foodPercent}%`,
...
const generateHardQuestions = (): TablesGraphQuestion[] => {
  const questions: TablesGraphQuestion[] = [];
...
export const generateAllTablesGraphQuestions = (): TablesGraphQuestion[] => {
...
if (require.main === module) {
  const allQuestions = generateAllTablesGraphQuestions();
  console.log('\nSample questions:');
